...
static void writeBatteryCharge(void)
{
	...
	printf("%s%s%%f%d%%%%  ", *statusBuffer == 'C' ? "%F{yellow}󱐋 " : "", charge <= 5 ?
		   "%F{red}  " : charge <= 25 ? "%F{yellow}  " : charge <= 50 ? "%F{green}  " :
		   "%F{green}  ", charge);
...
static void writeCalendar(struct tm* date)
{
	...
	printf("%%F{red}󰃭 %%f%s%s  ", buffer, IS_ORDINAL_DAY(1) ? "st" : IS_ORDINAL_DAY(2) ?
		   "nd" : IS_ORDINAL_DAY(3) ? "rd" : "th");
}

static void writeClock(struct tm* date)
{
	printf("%s%%f%02dh%02dm", date->tm_hour < 6 ? "%F{cyan}󰭎 " : date->tm_hour < 12 ?
		   "%F{red}󰖨 " : date->tm_hour < 18 ? "%F{blue} " : "%F{yellow}󰽥 ",
...
static void writeDiskUsage(void)
{
	...
	printf("%%F{%s}󰋊 %%f%d%%%%  ", usage < 70 ? "green" : usage < 80 ? "yellow" : "red",
		   usage);
