...
static void WriteClockModule(struct std::tm* localTime)
{
    std::cout << (localTime->tm_hour < 6    ? "%F{cyan}󰭎 "
                  ...
                  : localTime->tm_hour < 18 ? "%F{blue} "
                                            : "%F{yellow}󰽥 ")
              << "%f" << std::setfill('0') << std::setw(2) << localTime->tm_hour << "h"
              << std::setw(2) << localTime->tm_min << "m" << std::setfill(' ');
}
...
static void WriteDiskModule()
{
    ...
    int          use = (totalBytes - availableBytes) / static_cast<float>(totalBytes) * 100;
    if (use > 85)
    {
        std::cout << "%F{yellow}󰀦 ";
        g_modulesLength += 2;
...
static void WriteVirtualEnvModule()
{
    ...
    if (virtualEnv)
    {
        ...
        std::cout << "%f(" << virtualEnvStr.substr(virtualEnvStr.find_last_of("/") + 1) << ") ";
    }
}
